...
int valor = 0, impares = 0, cubo = 0, puntero = 0, parcial = 0;
	...
	for (int i = 1; i <= valor; i++){ // Recorre todos los números naturales desde 1 hasta el valor introducido
		puntero = (i*(i-1))+1; // Determina y acumula el impar a partir del cual comienza la suma de la serie de impares según el método de Nicómano.
		parcial = puntero; // Inicializa un valor intermedio con el puntero; puntero es el primer valor de la serie de i impares que se tienen que sumar; el siguiente bucle calculará los valores restantes de la serie y los acumulará en un valor parcial.
		cubo = puntero; // Se empieza a acumular valores para obtener el cubo; será la suma de todos los parciales.
		for (int j= 1; j < i; j++){
		...
		cout << "El cubo de " << i << " es igual a: " << cubo << endl; //Muestra para cada paso del bucle que recorre los números naturales el valor del cubo que da el bucle interior.
	}
